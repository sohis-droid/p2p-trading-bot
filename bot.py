...
async def check_deal_timeout(context: ContextTypes.DEFAULT_TYPE, room_num: int):
    ...
    if deal['status'] == 'init' and not deal.get('roles_selected'):
        ...
        timeout_msg = (
            f"⌛ DEAL ROOM EXPIRED\n\n"
            f"⌛ No activity for 5 minutes. Both parties have been removed from the deal room."
        )
        ...
        try:
            for uid in {
              deal.get('seller_id'),
              deal.get('buyer_id'),
              deal.get('joined_user_1'),
              deal.get('joined_user_2')
            }:
              if uid:
                try:
...
async def deal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not match:
        await update.message.reply_text("⚠️ Use: /deal @username")
        return
    ...
    try:
        ...
        lobby_msg = await update.message.reply_text(
            ...
            f"📝 Note: Only the mentioned members can join.\n"
            f"⚠️ Never join any link shared via DM.\n\n"
            f"🕒 Started: {current_time} IST\n"
            f"⏳ Auto-expires in 5 minutes if roles are not selected.",
...
async def msg_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == 'amount':
        try:
            ...
            await update.message.reply_text("⚠️ Invalid! Enter number")
        ...
        if success:
            ...
            kb = [[InlineKeyboardButton("⚠️ Verify Manually", callback_data=f'verify_{room_num}')]]
            ...
            for admin_id in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        ...
                        f"🔐 Escrow: {escrow}\n\n"
                        f"❌ Reason: {message}\n\n"
                        f"🔍 Please verify manually on the blockchain.",
                        reply_markup=InlineKeyboardMarkup(kb)
                    )
                except:
                    pass
        ...
        await context.bot.send_message(
            ...
            f"💵 After payment, click button below:",
            reply_markup=InlineKeyboardMarkup(kb)
        )
...
async def pay_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        parts = q.data.split('_')
        if len(parts) < 3:
            await q.message.reply_text("⚠️ Invalid payment selection. Please restart setup.")
            return
        ...
        if not selected_method:
            await q.message.reply_text("⚠️ Invalid payment mode selected.")
            return
        ...
        if missing:
            ...
            await q.message.reply_text("⚠️ Deal setup is incomplete. Please restart setup.")
            return
        ...
        if not escrow:
            ...
            await q.message.reply_text(f"⚠️ Escrow wallet not configured for {chain} {coin}. Contact admin.")
            return
        ...
        await q.message.reply_text("⚠️ Something went wrong while selecting payment mode. Admin please check logs.")
...
async def buyer_paid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        [InlineKeyboardButton("☑️ Received", callback_data=f'release_{room_num}')],
        [InlineKeyboardButton("⚠️ Dispute", callback_data=f'dispute_{room_num}')]
    ]
...
async def my_deals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for deal in deals[-10:]:
        role_emoji = "⚖️" if deal['role'] == 'seller' else "⚖️"
        msg += (
